...
public class CommonService {
	...
	public List<Map<String,Object>> findExistsMember(String jisaCD,String mkey,String mName,String birthYY){
		...
		return commonRepository.findExistsMember(map);
	}
	...
	public List<Map<String,Object>> findMujinMember(String jisaCD,String mkey,String mName,String birthYY){
		...
		map.put("mName",mName);
		map.put("birthYY",birthYY);
		return commonRepository.findMujinMember(map);
	}
	...
	public List<String> selectMJgrade(String jisaCD,String subj,String ipheiDay,String mkey,String loginLang){
		...
		if(mkey != null && !mkey.isEmpty()){
			MemSubjMst memSubjMst = commonRepository.findMemSubjMst(map);
			
			if(memSubjMst == null || memSubjMst.getMkey().isEmpty()){
				return commonRepository.selectMJgrade(map);
			}else{
				...
				long jsSetcnt = commonRepository.countJSET(map);
	...
	public Map<String,Object> ipheiSubjCheck(String jisaCD,String subj,String ipheiKind,int ipheiSeq,String mkey,String loginLang){
		...
		if("01".equals(ipheiKind)){
			...
			if(commonRepository.isExistsMemSubjMst(map)){
				String stateCD = commonRepository.selectStateCDMemSubjMst(map);
				if("1".equals(stateCD)){
					mkucode = "";
				}else{
					mkucode = "02";
			...
			if(commonRepository.isExistsMemMst(map)){
				if(commonRepository.isExistsMemSubjMst(map)){
					String stateCD = commonRepository.selectStateCDMemSubjMst(map);
					if("1".equals(stateCD)){
						mkucode = "";
		...
		if("KK".equals(subj)){
			...
			if(commonRepository.isExistsMemSubjMst(checkMap)){
				dongsubj = "KG";
				mkucode = "";
			}
		...
		if(commonRepository.isMemIpheiAgree(map)){
			agreeNum = commonRepository.findMemIpheiAgree(map);
			mkucode = "";
		}else{
			...
			if("KK".equals(subj)){
				...
				if(commonRepository.isMemIpheiAgree(daegiMap)){
					daegisubj = "KG";
					mkucode = "";
				}
			}else if("KG".equals(subj)){
	...
	public List<Map<String,Object>> selectDepositSet(String jisaCD,String depid1
		...
		return commonRepository.selectDepositSet(map);
	}
	...
	private boolean isDepositSet(String jisaCD,String depid1,String subj){
		...
		List<Map<String,Object>> list = commonRepository.isDepositSet(map);
	...
	public MemMst findMemMstById(String pMkey) {
		...
		return commonRepository.findMemMstById(map);
	}
	...
	public OmrGichoMujin findOmrGichoMujin(String pOmrdate, String pMkey,
		...
		return commonRepository.findOmrGichoMujin(map);
	}
	/**
	 * 회원번호 생성 NewSKey_INS
	 * @param ipKind
	...
	private long selectMemIpheiAgreeAgnum(){
		return commonRepository.selectMemIpheiAgreeAgnum();
	}
		...
		for (int i = 0; i < subj.length; i++) {
			if(!Strings.isNullOrEmpty(subj[i])){
				...
				if("FA".equals(empKeyLvCD) && (agvalFlag || (!"01".equals(mGubun[i]) && !"03".equals(mSayu[i]))) ){
					...
					Map<String,Object> empMap = commonRepository.selectComEmpMst(fstClass[i]);
					...
					commonRepository.insertMemIpheiAgree(memIpheiAgree);
					...
					//신입
						...
						boolean memCheckFlag = commonRepository.isExistsMemSubjMst(map);
						...
						if(!memCheckFlag){
							...
							commonRepository.insertMemSubjMst(memSubjMst);
							...
							commonRepository.insertMemMst(memMst);
							...
							commonRepository.insertMemIpheiMst(memIpheiMst);
							...
							commonRepository.insertMemIpgumMst(memIpgumMst);						
					...
					//복회	
						...
						if(memCheckFlag){
							...
							MemSubjMst memBeforSubjInfo = commonRepository.findMemSubjMst(map);
							...
							commonRepository.insertMemSubjMstHis(memSubjMstHis);
							...
							commonRepository.updateMemSubjMst(memSubjMst);
							...
							if(!memInsertCheckFlag){
								...
								MemMst memInfo = commonRepository.findMemMstById(memMap);
								...
								commonRepository.insertMemMstHis(memMstHis);
								//memMst 업데이트
								MemMst memMst = new MemMst();
								memMst.setmFstName(pFirstName);
								memMst.setmLstName("");
								...
								commonRepository.updateMemMst(memMst);
							}
